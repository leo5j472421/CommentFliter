...
def back(request) :
    error = True 
    if request.GET['youtube_address'] == 'Type Youtube video address...' :
        error = True 
    else :
        error = False
    ...
    if request.GET['youtube_address'] != '' and request.GET['youtube_address'] != ' ' :
        #get name of video
        url = str(request.GET['youtube_address'])
        url = url.replace(' ','')
        ...
        r=requests.get('https://www.googleapis.com/youtube/v3/commentThreads?part=snippet%2Creplies&maxResults=100&textFormat=plainText&videoId={ID}&key=AIzaSyAZqRuapzU5PE0wsV1dBWuQL2ePRP3DjaI'.format(ID= ID))
        t = json.loads(r.text)
...
def Feedback(request):
    ...
    print(answer)
    o = open('home/feedback.txt','a',encoding='utf-8-sig')
    o.write(answer+'\n')
    o.close()
